...
for l in alph_links:
    ...
    for a in book_links:
        ...
        #parent = soup.find('td', string='Год издания:')
        #year = parent.find_next('td').text
        srch_autor = drv.find_element(by = 'xpath', value = '//a[starts-with(@href, "//royallib.com/author/")]')
        h[srch_autor.get_attribute('href')] = srch_autor.get_attribute('text')
        #df = pd.DataFrame([h]).T
        ...
        #srch_ye = drv.find_element(by = 'xpath', value = '//td[starts-with(@b, "Год ")]')
        
    for s in dvl:
        books[s.get_attribute('href')] = s.get_attribute('text')
        df = pd.DataFrame({'books_name': [books], "autor": [h]}).T
