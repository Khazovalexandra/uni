...
class Main(Frame):
    ...
    def startUI(self):
        ...
        self.lbl = Label(root, text="Начало игры!", bg="#ffef9c",
...
def socket_start():
    ...
    def client(sock):
        #while True:
        print(app.get_opponent_choise())
        while app.get_opponent_choise() != 0:
            message = str(app.get_opponent_choise())
            print(message)
    ...
    socket1_thread = threading.Thread(target=client, args=(conn, ))
    socket2_thread = threading.Thread(target=server, args=(sock1, ))
